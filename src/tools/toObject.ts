...
export const ERRORS = Symbol('todo:errors');
...
function _toObject<TTarget extends object, TFormat, Tkey extends PropertyKey>(
  ...
  if (validationError.issues.length) {
    ...
    for (const issue of validationError.issues)
      object[errorKey].push(format(issue, validationError));
  }
...
export function toObject<
  TTarget extends object,
  TFormat = string,
  TKey extends PropertyKey = typeof ERRORS,
>(
  validationError: ValidationError<TTarget>,
  ...
  return _toObject(
    validationError,
    format ?? (defaultFormat as ToObjectIssueFormat<TFormat>),
    key,
  );
}
